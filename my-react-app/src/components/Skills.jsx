...
const Skills = () => {
  ...
  return (
    <section id="skills" className="py-20 px-6 md:px-20 text-white">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-10 mt-8">
        {icons.map((icon, index) => (
          <div
            key={index}
            className="p-8 rounded-xl border border-gray-700 shadow-2xl flex flex-col items-center justify-center hover:scale-105 transition-transform duration-300 ease-in-out"
          >
            <img
              src={icon.src}
              alt={icon.name}
            ...
            />
            <p className="text-lg font-semibold text-gray-200">{icon.name}</p>
          </div>
        ))}
      </div>
    </section>
